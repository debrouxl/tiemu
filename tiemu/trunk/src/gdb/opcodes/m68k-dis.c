  ...
  if ((word & 0x100) == 0)
    {
      ...
      if ((base_disp & 0x80) != 0)
	base_disp -= 0x100;
      (*info->fprintf_func) (info->stream, "%d", (int)base_disp);
      if (basereg == -1)
        {
	base_disp += addr;
          (*info->fprintf_func) (info->stream, " [");
        ...
        {
          (*info->fprintf_func) (info->stream, "(%s", reg_names[basereg]);
        }
      (*info->fprintf_func) (info->stream, ",%s)", buf);
      return p;
    }
  ...
  switch (*d)
	...
	case 7:
	  switch (val & 7)
	    ...
	    case 2:
	      val = NEXTWORD (p);
	      (*info->fprintf_func) (info->stream, "%d(%%pc) [", (int)val);
          ...
          switch (op)
            {
              ...
              case 0xffee:  /* jmp __ld_entry_point_plus_0x8000+word */
                pm = NEXTWORD(buffer);
                {
                  ...
                  heap_search_for_address(memaddr + 2, &handle);
