...
int write_bmp_2_colors(FILE *file, Image *img) //tested: OK (09/04/2002)
{
  ...
  int           NbBytesColorTable, NbColors;
  DWORD        *ColorTable;
  int           NbBytesImage;
  BYTE         *BitsImage, *TrueBitsImage;
  int           Err;
  int           i;     
  ...
  ColorTable = (DWORD *)malloc(NbBytesColorTable);
  ...
  TrueBitsImage = (BYTE *)calloc(BmpInfoHeader.biSizeImage+BytesPerLine,sizeof(unsigned char)); 
