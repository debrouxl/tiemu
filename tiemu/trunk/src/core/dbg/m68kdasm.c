...
char* sym_addr(uint32_t addr)
{
	...
	if(rcid == -1)
		...
		snprintf(buf, sizeof(buf), "$%06X -> tios::%s", addr, romcalls_get_name(rcid)); 
...
uae_s32 ShowEA (FILE *f, int reg, amodes mode, wordsizes size, char *buf)
{
	...
	sprintf (buffer,"(A%d,$%04X) [%s]", reg, disp16 & 0xffff,
					sym_addr((unsigned long)addr));
	break;
     case Ad8r:
	...
	if (dp & 0x100) {
	    ...
	    addr = base + outer;
	    sprintf (buffer,"(%s%c%d.%c*%d+%ld)+%ld [$%06x]", name,
	  ...
	  sprintf (buffer,"(A%d, %c%d.%c*%d, $%02X) [$%06x]", reg,
	       dp & 0x8000 ? 'A' : 'D', (int)r, dp & 0x800 ? 'L' : 'W',
	       1 << ((dp >> 9) & 3), disp8,
	       (unsigned long)addr);
	}
	...
	disp16 = get_iword_1 (m68kpc_offset); m68kpc_offset += 2;
	addr += (uae_s16)disp16;
	sprintf (buffer,"[%s]", sym_addr((unsigned long)addr));
	break;
     case PC8r:
	addr = m68k_getpc () + m68kpc_offset;
	dp = get_iword_1 (m68kpc_offset); m68kpc_offset += 2;
	...
	if (dp & 0x100) {
	    ...
	    addr = base + outer;
	    sprintf (buffer,"(%s%c%d.%c*%d+%ld)+%ld [%s]", name,
		    dp & 0x8000 ? 'A' : 'D', (int)r, dp & 0x800 ? 'L' : 'W',
		    1 << ((dp >> 9) & 3),
		    disp,outer,
		    sym_addr((unsigned long)addr));
	  ...
	  sprintf (buffer,"(PC, %c%d.%c*%d, $%02X) [%s]", dp & 0x8000 ? 'A' : 'D',
		(int)r, dp & 0x800 ? 'L' : 'W',  1 << ((dp >> 9) & 3),
		disp8, sym_addr((unsigned long)addr));
	}
	break;
     case absw:
